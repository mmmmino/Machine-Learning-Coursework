...
class GradientDescent:
    ...
    def fit(self, features, targets, batch_size=None, max_iter=1000):
        ...
        N = features.shape[0]
        d = features.shape[1]
        self.model = np.random.random(d+1)*0.2 - 0.1
        if batch_size is None:
            batch_size = N

        bias = np.ones(N)
        features = np.insert(features, obj=d, values=bias, axis=-1)
        ...
        while not convergence:
            for index in range(0, N, batch_size):
                # get current batch
                X, y = features[index:index+batch_size], targets[index:index+batch_size]
                # forward pass before update
                loss_before_update = self.loss.forward(features, self.model, targets)
                # update the model parameters
                self.model -= self.learning_rate * self.loss.backward(X, self.model, y)
                # forward pass after update
                loss_after_update = self.loss.forward(features, self.model, targets)
                step += 1
                # judge convergence
                if np.abs(loss_before_update - loss_after_update) < 1e-4 or step >= max_iter:
                    convergence = True
                    break
    ...
    def predict(self, features):
        ...
        Returns:
            predictions - (np.array) A 1D array of predictions of length N,
                ...
                features.
        """
        predictions = self.confidence(features)
